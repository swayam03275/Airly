...
import {
  invalidateCacheOnPostCreate,
  invalidateCacheOnPostDelete,
  invalidateCacheOnPostUpdate,
...
const createTweet = asyncHandler(async (req, res) => {
  ...
  // Invalidate feed cache when new tweet is created
  await invalidateCacheOnPostCreate(processedTags);
...
const deleteTweet = asyncHandler(async (req, res) => {
  ...
  // Invalidate feed cache when tweet is deleted
  await invalidateCacheOnPostDelete(tweetTags);
...
const editTweet = asyncHandler(async (req, res) => {
  ...
  // Invalidate feed cache when tweet is updated
  const updatedTags = tweet.tags || [];
  await invalidateCacheOnPostUpdate(updatedTags);
...
const searchContent = asyncHandler(async (req, res) => {
  ...
  if (searchType === "all" || searchType === "tweets") {
    ...
    results.nextCursor = results.hasMore
      ? results.tweets[results.tweets.length - 1]._id
      : null;
  }
